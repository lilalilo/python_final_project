...
class ActivationVisualizer:
    ...
    #     # 엣지 마스크  
    #     axes[0,2].imshow(edge_mask.squeeze().cpu(), cmap='hot', vmin=0, vmax=1)
    #     axes[0,2].set_title('Edge Mask')
        
    #     # 거리 맵
    #     axes[1,0].imshow(distance_map.squeeze().cpu(), cmap='viridis', vmin=0, vmax=1)
    #     axes[1,0].set_title('Distance Map')
        
    #     # 포인트 맵
    #     axes[1,1].imshow(point_map.squeeze().cpu(), cmap='plasma', vmin=0, vmax=1)
    #     axes[1,1].set_title('Point Map')
        
    #     # 최종 마스크
    #     im = axes[1,2].imshow(final_mask.squeeze().cpu(), cmap='jet', vmin=0, vmax=1)
    #     axes[1,2].set_title('Final Mask (Sampling Weights)')
    #     plt.colorbar(im, ax=axes[1,2])
